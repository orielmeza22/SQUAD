...
def architect_prompt(prompt: str, search_ctx: str, preflight: dict, existing_context: str = "") -> str:
    ...
    return (
        ...
        "  * ⚠️ REGLA SQUAD 2.0: Todo el backend de FastAPI (modelos, rutas, base de datos y uvicorn) DEBE estar estructurado en un único archivo: main_output.py. Está PROHIBIDO planificar carpetas backend/ o archivos secundarios como crud.py o models.py. Todo debe ser autocontenido en el punto de entrada.\n"
        "- Si el stack seleccionado es NODE_EJS:\n"
        "  * El punto de entrada DEBE ser server.js.\n"
        "  * Se permite usar package.json y plantillas ejs.\n"
        "  * ⚠️ REGLA SQUAD 2.0: Toda la lógica de Express y SQLite del servidor DEBE estar estructurada en un único archivo: server.js. No planifiques subcarpetas de backend.\n"
        "- Si el stack seleccionado es PYTHON_STREAMLIT:\n"
        "  * El punto de entrada DEBE ser app.py.\n"
        "  * ⚠️ REGLA SQUAD 2.0: Toda la lógica y UI de Streamlit DEBE estar estructurada en un único archivo: app.py.\n\n"
        ...
        "⚠️ REGLA DE ESPECIFICACIÓN (SQUAD 2.0): NO escribas código de implementación completo (como bloques de Python o JS de más de 10 líneas) dentro de SPEC.md. Solo debes definir la estructura de las tablas de base de datos, el listado de endpoints con sus métodos HTTP, rutas e inputs/outputs esperados en formato descriptivo de documentación. Esto evita truncamientos por límites de tokens y permite al Agente Backend encargarse de la codificación real.\n"
