...
def architect_prompt(prompt: str, search_ctx: str, preflight: dict, existing_context: str = "") -> str:
    ...
    return (
        ...
        "REGLA DE PREVISUALIZACIÓN: Si planificas una previsualización en el navegador local, "
        "la página principal (index.html) debe ser un HTML5 estándar ejecutable directamente en "
        "el navegador sin bundler (o usar librerías como Vue/React/Tailwind vía CDN), a menos que "
        "el entorno soporte y configure explícitamente un entorno Node/NPM.\n"
...
def code_review_prompt(plan: str, created_files: list) -> str:
    ...
    return (
        ...
        "Señala si hay errores graves, importaciones faltantes, o asimetrías de puertos "
        "(ejemplo 3000 vs 8000). Si encuentras algo crítico, escribe SÍ_CRITICO y qué falló."
    )
